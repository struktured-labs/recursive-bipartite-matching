...
EMD-trained bot head-to-head. On No-Limit Hold'em, RBM wins
abstraction quality 5--0 at 20bb (up to 73\% less error) and dominates
at fine-grained compression at 200bb deep-stack (60\% less error at
$k{=}25$), confirming that RBM's structural advantage persists across
game complexity scales.
\end{abstract}
...
Island Hold'em, 5--0 on Limit Hold'em, and 5--0 on No-Limit Hold'em
20bb across compression levels. On No-Limit, RBM achieves 73\% less
error than EMD at $k{=}25$ on 20bb and 60\% less error at 200bb deep
stacks, with its advantage strongest at fine-grained compression. An
MCCFR bot trained on RBM abstractions defeats the EMD-trained bot
head-to-head over 20{,}000 hands in Limit. The online learner achieves
99.4\% cache hit rates with ${\sim}400$ naturally emerging postflop
clusters.
...
($k{\leq}10$). The EMD advantage at coarse compression reflects a
limitation of the experimental setup: the showdown distribution trees
used for tractable pairwise RBM distance (${\sim}$161 nodes) do not
fully capture the structural variation of the 186{,}174-node game tree.
...
both depths, but the 200bb result is confounded by severe
undertraining (${\sim}$1M info sets with only 50K iterations).}
\label{tab:nl-cfr}
\begin{tabular}{lccr}
\toprule
...
matches. At 20bb, the margin is small ($-0.12$ bb/hand) and consistent
with the push/fold nature of short-stack play where raw equity is a
strong predictor of optimal play. At 200bb, the margin is larger
($-1.05$ bb/hand) but comes with a critical caveat: the 200bb game
...
was still fluctuating at training end, far from convergence. The 200bb
game requires 500K+ iterations for meaningful head-to-head comparison.
...
\midrule
20bb & 2{,}988 & RBM 5--0 & EMD +0.12 bb/h & push/fold \\
200bb & 186{,}174 & RBM 2--3 & EMD +1.05 bb/h & undertrained \\
\bottomrule
\end{tabular}
\end{table}
...
The results span three domains at multiple scales and tell a consistent
story. On Rhode Island Hold'em, RBM beats EMD 7--0 with
zero-exploitability at natural cluster boundaries. On Limit Hold'em
with a 52-card deck~\citep{bowling2015solving}, RBM beats EMD 5--0
across all compression levels (up to 42\% less error), and an MCCFR bot
trained on RBM abstractions defeats the EMD bot head-to-head. On
No-Limit Hold'em at 20bb, RBM wins abstraction quality 5--0 with up to
73\% less error, though EMD prevails in head-to-head play on short
stacks where raw equity dominates. At 200bb (186{,}174 nodes), RBM
still dominates at fine-grained compression (60\% less error at
$k{=}25$, 40\% at $k{=}15$), though the head-to-head comparison is
confounded by severe MCCFR undertraining (${\sim}$1M info sets with
only 50K iterations). The pattern across domains confirms the central
thesis: structural tree distance is a better abstraction metric than
scalar hand-strength distributions, with RBM's advantage most
pronounced at fine-grained compression where structural distinctions
matter most.

Several directions for future work remain. \emph{Adequate training at
200bb:} with ${\sim}$1M info sets, the 200bb game requires 500K+
MCCFR iterations for meaningful head-to-head comparison; the current
50K-iteration results are dominated by training noise rather than
abstraction quality. \emph{Full-tree RBM:} the showdown distribution
trees used for tractable pairwise RBM distance (${\sim}$161 nodes) lose
discriminative power compared to the full 186{,}174-node game trees;
approximate matching via Sinkhorn
distances~\citep{cuturi2013sinkhorn} or learned embeddings could enable
RBM on the full trees while preserving error bounds.
\emph{Multi-player:} the implementation supports 2--10 players, and
6-max 20bb already produces ${\sim}$13M-node game trees where
equity-based clustering is increasingly inadequate. \emph{Beyond
