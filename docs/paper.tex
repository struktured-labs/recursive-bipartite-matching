...
structurally distinct subtrees (C). Poker robustly violates (A).}
\label{tab:boundary}
\small
\begin{tabular}{llcc}
\toprule
...
\paragraph{Synthesis.} The recursive tree distance is not a
...
Heads-up poker, our original target, robustly violates~(A): equity is a
strong sufficient statistic, which is why RBM equals EMD on Rhode
Island and why its finer-grained advantage on No-Limit does not survive
into a head-to-head edge. The conditions are, however, naturally
satisfied by \emph{risk-sensitive sequential decision problems}, where
the full outcome distribution and the conditional path structure both
drive the optimal policy---marking the more promising domain for
recursive tree-distance abstraction. We emphasize that these are small,
exactly-solvable diagnostics chosen to expose the mechanism; scaling
them into a full risk-sensitive control benchmark is future work.
