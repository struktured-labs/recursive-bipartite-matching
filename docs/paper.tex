...
($k \le 10$). Two notes are in order. First, the EMD advantage at
coarse compression reflects a limitation of the experimental setup:
the showdown-distribution trees used for tractable pairwise RBM
distance (${\sim}$161 nodes) do not fully capture the structural
variation of the 186{,}174-node game tree. The 200bb results in this
table should therefore be read as a lower bound on what full-tree RBM
would achieve at this stack depth---the headline 5--0 framing we use
elsewhere does \emph{not} apply here, and we do not claim it.
Second, the non-monotonic pattern (RBM wins at fine $k$, loses at
coarse $k$) is consistent with the abstraction-pathology phenomenon
documented by \citet{waugh2009abstraction}: a more refined abstraction
can produce a worse-playing strategy, depending on how the
coarse-graining interacts with the underlying game's strategic
structure. We flag this rather than wave it away, because it is the
kind of pattern an exploitability-aware evaluation
(\citealt{johanson2013evaluating}) is best suited to disentangle.
...
\emph{Exploitability evaluation along
\citet{johanson2013evaluating}:} the most important missing piece is
a direct exploitability measurement of RBM-induced strategies, which
would (a)~test whether the abstraction-quality EV-error advantage
predicts a tighter exploitability bound, and (b)~detect any
abstraction-pathology behaviour~\citep{waugh2009abstraction} that the
EV-error metric does not surface.
\emph{Tighter EV-to-exploitability bridge:} extending
Theorem~\ref{thm:ev-error} into a Kroer--Sandholm-style
exploitability bound (cf.\ Remark~\ref{rem:ev-vs-expl}) is the natural
