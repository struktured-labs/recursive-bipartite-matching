...
EMD-trained bot head-to-head. On No-Limit Hold'em (20bb heads-up),
RBM wins abstraction quality 3--2, with up to 64\% less error at
fine-grained compression ($k{=}25$), confirming that RBM's structural
advantage grows with game complexity.
\end{abstract}
...
validate the theory: RBM-based clustering beats EMD 7--0 on Rhode
Island Hold'em, 5--0 on Limit Hold'em, and 3--2 on No-Limit Hold'em
across compression levels. On No-Limit, RBM achieves 64\% less error
than EMD at $k{=}25$, with its advantage growing at finer-grained
compression. An MCCFR bot trained on RBM abstractions defeats the
EMD-trained bot head-to-head over 20{,}000 hands in Limit. The online
learner achieves 99.4\% cache hit rates with ${\sim}400$ naturally
emerging postflop clusters.
...
We extend the evaluation to heads-up No-Limit Hold'em with 20 big blind
stacks (NL-HU 20bb), the game variant underlying modern superhuman
...
(50 canonical hands). Mean EV error at each cluster count. Lower is
better. \textbf{RBM wins 3--2}, dominating at fine-grained compression.}
\label{tab:nl-preflop}
\begin{tabular}{rcccl}
\toprule
$k$ & RBM Error & EMD Error & Improvement & Winner \\
\midrule
25 & 0.12 & 0.34 & 64\% less error & RBM \\
15 & 0.21 & 0.36 & 41\% less error & RBM \\
10 & 0.22 & 0.36 & 39\% less error & RBM \\
 5 & 0.52 & 0.39 & --- & EMD \\
 3 & 0.53 & 0.54 & --- & tie (EV) \\
\bottomrule
\end{tabular}
...
Table~\ref{tab:nl-preflop} shows the results. \textbf{RBM wins 3--2}
across compression levels. The pattern is revealing: RBM dominates at
fine-grained compression (64\% less error at $k{=}25$, 41\% at
$k{=}15$, 39\% at $k{=}10$) but loses at coarse compression ($k{=}5$).
At $k{=}3$, the two metrics are essentially tied (0.53 vs.\ 0.54).

This result is honest about RBM's limitations in the short-stack
setting: 20bb No-Limit is heavily push/fold oriented, where raw equity
is a strong predictor of optimal play. EMD's equity-based clustering is
well-suited to this regime. RBM's structural advantage---capturing the
shape of continuation trees, not just expected values---becomes
increasingly important as stack depth grows and post-flop play
deepens. The 64\% improvement at $k{=}25$ demonstrates that even in
short-stack NL, RBM finds meaningful structural distinctions that
equity alone misses when enough clusters are available.

\subsection{No-Limit Hold'em: MCCFR Head-to-Head}

We train MCCFR bots on the NL-HU 20bb game using RBM and EMD preflop
abstractions, then evaluate in a 20{,}000-hand position-alternated
match.

\begin{table}[t]
\centering
\caption{MCCFR head-to-head on NL-HU 20bb (20{,}000 hands,
position-alternated). EMD wins on short stacks where push/fold
decisions are dominated by raw equity.}
\label{tab:nl-cfr}
\begin{tabular}{lc}
\toprule
Matchup & Result (bb/hand) \\
\midrule
RBM bot vs EMD bot & $-0.48$ (EMD wins) \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:nl-cfr} shows that the EMD bot wins the head-to-head at
$+0.48$ bb/hand. This is consistent with the abstraction quality
results: at the coarse preflop level where MCCFR operates, EMD's equity
clustering is effective for short-stack play. The key insight is that
RBM's structural advantage grows with game complexity---deeper stacks
(200bb: 186{,}174 nodes vs.\ 20bb: 2{,}988 nodes), more players
(6-max: ${\sim}$13M nodes), and post-flop streets where continuation
tree structure diverges from pure equity. We expect RBM to outperform
EMD in these richer settings, where the 64\% error reduction at
$k{=}25$ would translate to stronger play.
...
The results span three domains and tell a consistent story. On Rhode
Island Hold'em, RBM beats EMD 7--0 with zero-exploitability at
natural cluster boundaries. On Limit Hold'em with a 52-card
...
RBM abstractions defeats the EMD bot head-to-head. On No-Limit
Hold'em, RBM wins abstraction quality 3--2 with up to 64\% less error
at fine-grained compression, though EMD prevails in head-to-head play
on short stacks where raw equity dominates. The pattern across domains
confirms the central thesis: structural tree distance is a better
abstraction metric than scalar hand-strength distributions, with RBM's
advantage growing as game complexity increases.

The No-Limit results also highlight a clear direction for future
work. \emph{Deep-stack NL:} at 200bb (186{,}174 nodes vs.\ 2{,}988 at
20bb), post-flop play is rich enough that RBM's structural advantage
should translate to stronger head-to-head results. \emph{Multi-player:}
the implementation supports 2--10 players, and 6-max 20bb already
produces ${\sim}$13M-node game trees where equity-based clustering is
increasingly inadequate. \emph{Scalability:} the $O(n^2)$ distance
computation limits application to large games; approximate matching via
Sinkhorn distances~\citep{cuturi2013sinkhorn} or learned embeddings
could help while potentially preserving error bounds. \emph{Beyond
poker:} the framework applies to any extensive-form game with
tree-structured state spaces, including Stratego, Magic: The Gathering,
and negotiation games. \emph{Opponent modeling:} building an RBM-based
EV graph of the opponent's play patterns could enable principled
exploitation.
