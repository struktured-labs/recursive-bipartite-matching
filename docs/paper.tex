...
\author{
  Anonymous Authors\thanks{Work in progress. Correspondence to the authors.}
}
...
compression levels measured by EV error, and counterfactual regret
minimization on RBM-compressed games achieves zero exploitability at
...
Experiments on Rhode Island Hold'em~\citep{gilpin2005rhode}
(Section~\ref{sec:experiments}) validate the theory: RBM-based
clustering beats EMD-based clustering 7--0 across compression levels on
EV error, CFR on compressed games achieves zero exploitability at
natural cluster boundaries, and the online learner converges to stable
clusters within 100 games with 99.5\% cache hit rates.
...
We evaluate on Rhode Island Hold'em~\citep{gilpin2005rhode}, a
simplified poker variant designed for AI research: 2 players, 1 hole
card each, 2 community cards (flop + turn), 3 betting rounds with limit
betting, and 3-card hand rankings (trips $>$ straight $>$ flush $>$
pair $>$ high card).

All experiments use a configurable deck (2--4 ranks, 4 suits). The
implementation is in OCaml using Jane Street Core libraries, with an
$O(n^3)$ Hungarian algorithm for bipartite matching.
...
counts (3-rank Rhode Island Hold'em, 25 sampled deals). Lower is
better. RBM wins at every non-trivial compression level.}
\label{tab:rbm-vs-emd}
\begin{tabular}{rccccl}
\toprule
Clusters & Compress & RBM Error & EMD Error & EV Error & Winner \\
\midrule
25 & 1.0$\times$ & 0.0000 & 0.0000 & 0.0000 & tie \\
20 & 1.2$\times$ & 0.3571 & 6.1548 & 3.2143 & RBM \\
15 & 1.7$\times$ & 0.7143 & 10.5357 & 6.4286 & RBM \\
10 & 2.5$\times$ & 2.0536 & 12.6786 & 9.6429 & RBM \\
 7 & 3.6$\times$ & 4.2857 & 17.7976 & 14.6429 & RBM \\
 5 & 5.0$\times$ & 7.1429 & 25.3571 & 18.9286 & RBM \\
 3 & 8.3$\times$ & 12.1429 & 33.9286 & 21.4286 & RBM \\
 2 & 12.5$\times$ & 17.1429 & 37.5000 & 28.2143 & RBM \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:rbm-vs-emd} shows results. \textbf{RBM wins 7--0}
across all non-trivial compression levels. At moderate compression
(5~clusters, 5$\times$), EMD produces roughly 3.5$\times$ more EV error
than RBM; at aggressive compression (2~clusters, 12.5$\times$), EMD
error is 2.2$\times$ higher. The key insight: boards with similar
showdown distributions can have very different optimal play when the
game tree structure differs, and RBM captures these structural
differences.
...
Hold'em (3-rank, 2 betting rounds, 1000 CFR iterations). Zero
exploitability at natural cluster boundaries confirms that RBM
identifies strategically equivalent states.}
\label{tab:cfr}
\begin{tabular}{rrrr}
\toprule
...
\midrule
(full) & 10 & --- & 0.000034 \\
0 & 10 & 0.0000 & 0.000034 \\
50 & 8 & 0.0000 & 0.000000 \\
100 & 7 & 0.0000 & 0.000000 \\
200 & 5 & 18.75 & 3.125000 \\
500 & 3 & 26.25 & 6.041667 \\
1000 & 1 & 40.00 & 10.833333 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:cfr} reveals a striking pattern. At $\varepsilon = 50$
and $\varepsilon = 100$, compression is nontrivial (8 and 7 clusters
from 10 original information sets) yet exploitability is \emph{exactly
zero}---the RBM distance has identified genuinely equivalent strategic
states. Exploitability increases only when compression begins merging
strategically distinct hands ($\varepsilon \ge 200$), consistent with
the EV error bound.
...
The 7--0 victory of RBM over EMD on Rhode Island Hold'em validates the
central thesis: structural tree distance is a better abstraction metric
than scalar hand-strength distributions. The zero-exploitability results
at natural cluster boundaries confirm that RBM identifies genuinely
equivalent strategic states.
...
\paragraph{Part 1: Exploration bound.}
...
\paragraph{Part 2: Per-step exploitation error.}
...
\[
  ...
  \le \frac{\varepsilon}{2}. \qed_2
\]

\paragraph{Part 3: Cumulative regret bound.}
...
\[
  ...
  + (T - K)\frac{\varepsilon}{2}. \qed_3
\]

\paragraph{Part 4: Average regret convergence.}
\[
...
\paragraph{Part 5: Sublinear regret via $\varepsilon$-annealing.}
