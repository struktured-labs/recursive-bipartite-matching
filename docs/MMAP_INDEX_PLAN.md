...
|---|---|---|---|
| 1 — u32 fingerprints at MPHF slots | ✅ shipped | #4 | 4 B/key fast-path filter; opt-out backward-compat via `Option<Arena<u32>>` |
| 2 — PtrHash vs `ph::fmph` bench | ✅ shipped | #5 | PtrHash 7-10× faster lookup, +0.3 bits/key (negligible); dev-box only, re-run on prod EPYC before Phase 4 adoption |
| 3 — Freeze-time RBP-style pruning | ✅ shipped | #6 | Opt-in via `TrainConfig.freeze_prune_{regret,strategy}_threshold`; defaults 0/0 = no behavior change |
| 6 — `MADV_HUGEPAGE` + `MPOL_INTERLEAVE` | ✅ shipped | #7 | 2 MiB transparent huge pages + NUMA interleave at startup; no-op on single-node boxes |
| 4 — Hot-tier snapshot freeze + `arc-swap` layer publish | 🟡 deferred | — | Current `CompactCfrState` is per-thread → no shared-reader to protect. Only valuable as Phase 5 foundation. Revisit after prod-hardware perf baseline. |
| 5 — Per-generation arenas + bundled lifetime | 🟡 deferred | — | Highest-risk phase per verifier (concurrency + lifetime invariants). Defer until empirical 1B-iter baseline on the new EPYC 7402P box shows arena reclamation is the bottleneck. |

Phases 1+2+3+6 collectively address the dev-box-measurable wins. Phases 4+5 are the architecture-changing phases that should wait for production hardware data + a more careful concurrency design (the original synthesis had 2 verified showstoppers in this area).
